...
class GameApi {
  static const _base = 'http://192.168.0.5:8080';
  ...
  static Future<GameResponse> getGameById(String gameId) async {
    final uri = Uri.parse('$_base/games/$gameId');
    final resp = await http.get(uri, headers: {'Content-Type': 'application/json'});
    if (resp.statusCode == 200) {
  ...
  /// 4) 상세 정보 즉시 갱신 요청 (응답 메시지만)
  static Future<String> updateGameDetail(String gameId) async {
    final uri = Uri.parse('$_base/games/$gameId/detail/update');
  ...
  static Future<void> saveRecord({
    ...
    final uri = Uri.parse('$_base/records'); // 실제 엔드포인트로 변경
